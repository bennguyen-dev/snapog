...
export const GenerateService = () => {
    ...
    const generateOGImage = async (url: string): Promise<{ url: string, base64Image: string } | null> => {
        try {
            ...
            const response = await page.goto(url, { waitUntil: 'networkidle2', timeout: 60000 }); // Set timeout to 60 seconds
    ...
    const generateOGImagesForDomain = async (domain: string): Promise<{ url: string, base64Image: string }[]> => {
        try {
            ...
            const screenshotPromises = pagesToCapture.map(url => generateOGImage(url));
